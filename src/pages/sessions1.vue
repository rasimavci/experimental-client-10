...
f7-page
  ...
  f7-list.date(v-for='(groups, key) in groupedCalls' :key="key")
   ...
   f7-list
    f7-list-item.my-class(v-for="conv in getConversations" @click='goCall(group)' :key="conv.name" :title="conv.destinationId + ' ' + conv.destinationId" href="#popupAddContact")
</template>
<script>
